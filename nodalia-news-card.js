...
(() => {
  ...
  var NEWS_HISTORY_HELPER_MAX_CHARS = 250;
  var NEWS_HISTORY_HELPER_WRITE_MS = 450;
  var MAGAZINE_SLIDE_TRANSITION_MS = 520;
  var ITEM_LIST_ATTRS = ["items", "articles", "entries", "news", "headlines"];
  var LAYOUT_MODES = /* @__PURE__ */ new Set(["compact", "magazine", "list"]);
  ...
  function normalizeConfig(rawConfig) {
    ...
    merged.layout = {
      ...
      density: DENSITY_MODES.has(String(layout.density || "").trim()) ? String(layout.density).trim() : DEFAULT_CONFIG.layout.density,
  ...
  function isSafeHttpUrl(url) {
    ...
    try {
      const base = typeof window !== "undefined" && window.location?.origin ? window.location.origin : void 0;
  ...
  function normalizeNewsItem(raw, sourceMeta = {}) {
    ...
    const source = pickFirstString(raw, ["source", "publisher", "feed", "author"]) || String(sourceMeta.name || "").trim();
    const category = pickFirstString(raw, ["category", "section", "tag"]) || String(sourceMeta.category || "").trim();
    const url = pickFirstString(raw, ["url", "link"]);
    const image = sanitizeImageUrl(pickFirstString(raw, ["image", "image_url", "thumbnail", "picture"]));
    const publishedRaw = pickFirstString(raw, ["published", "published_at", "date", "datetime", "timestamp"]) || raw.published || raw.published_at || raw.date || raw.datetime || raw.timestamp;
  ...
  function coerceNewsAttributeList(value) {
    ...
    if (isObject(value)) {
      ...
      const numericEntries = Object.keys(value).filter((key) => /^\d+$/.test(key)).sort((left, right) => Number(left) - Number(right)).map((key) => value[key]).filter((entry) => entry && typeof entry === "object");
  ...
  function encodeCompactNewsHistoryEntry(item) {
    ...
    const entry = {
      ...
      p: compact.publishedMs ?? void 0,
      e: compact.sourceEntityId || void 0,
      u: compact.url ? compact.url.slice(0, 56) : void 0,
      s: compact.source ? compact.source.slice(0, 24) : void 0,
      c: compact.category ? compact.category.slice(0, 20) : void 0,
      m: compact.summary ? compact.summary.slice(0, 48) : void 0,
      g: compact.image ? compact.image.slice(0, 56) : void 0
    };
    Object.keys(entry).forEach((key) => {
      if (entry[key] === void 0 || entry[key] === "") {
        delete entry[key];
      }
    });
    return entry;
