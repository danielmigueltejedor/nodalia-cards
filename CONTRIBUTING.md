...
| **`alpha`** | Developers / brave testers | **`0.6.0-alpha.1`**, **`0.6.0-alpha.2`**, … (tags **`v0.6.0-alpha.1`**, … optional) | **High churn**. Frequent commits; **dashboards may break**. Breaking YAML or behaviour is allowed here. |
...
# set package.json to e.g. 0.6.0-alpha.1, changelog section, npm run bundle
...
3. **`alpha` (experimental)** — work and tag **`0.6.0-alpha.1`**, **`0.6.0-alpha.2`**, … as often as you like; merge **`alpha` → `beta`** when a feature is **ready for testers**, then bump **`beta`** (**`0.6.0-beta.2`**, **`beta.3`**, …).
