...
![Alpha branch](https://img.shields.io/badge/alpha-0.4.0--alpha.1-orange)
...
**Prereleases:** **`beta`** (`0.4.0-beta.1`, `beta.2`, …) — likely usable; for testers. **`alpha`** (`0.4.0-alpha.1`, `alpha.2`, …) — frequent changes; **may break** dashboards. Experimental work lands on **`alpha`**, then moves to **`beta`** when a feature is polished enough; stable minors ship from **`main`** only. Details and tagging: **CONTRIBUTING**.
