/* Generated from src/cards/power-flow. Do not edit. */
"use strict";
(() => {
  ...
  var SVG_PATH_TOKEN_RE = /[AaCcHhLlMmQqSsTtVvZz]|[+-]?(?:(?:\d*\.\d+)|(?:\d+\.?))(?:[eE][+-]?\d+)?/g;
    ...
    while (reader.hasMore()) {
      ...
      if (upper === "A") {
        while (reader.hasNumber()) {
          ...
          const x = reader.readNumber();
          const y = reader.readNumber();
          if (!Number.isFinite(rx) || !Number.isFinite(ry) || !Number.isFinite(rotation) || !Number.isFinite(largeArc) || !Number.isFinite(sweep) || !Number.isFinite(x) || !Number.isFinite(y)) {
            return { start, path: "M 0 0" };
          }
          output.push(
            command,
  ...
  function isEntitySourceConfigured(entity) {
    ...
    if (isObject(entity)) {
      return Boolean(
        String(entity.entity || "").trim() || String(entity.consumption || "").trim() || String(entity.production || "").trim()
      );
    }
    return false;
  }
  ...
  function getNodePositionForLayout(kind, index = 0, total = 0, hasBottomUtilities = false, layoutPreset = "full", flowFlags = {}) {
    ...
    if (kind === "solar") {
      const upperBandSolo = flags.hasSolar && flags.hasGrid && !flags.hasBattery && bottomN === 0 && individualCount === 0;
  ...
  function buildFlowPath(from, to, fromRadius = 0, toRadius = 0, hints = {}) {
    ...
    if (horizFirst) {
      const paX2 = end.x - sx * r;
      const paY2 = start.y;
      const pbX2 = end.x;
      const pbY2 = start.y + sy * r;
      const sweep2 = sx * sy > 0 ? 1 : 0;
      return `M ${start.x.toFixed(2)} ${start.y.toFixed(2)} L ${paX2.toFixed(2)} ${paY2.toFixed(2)} A ${r.toFixed(2)} ${r.toFixed(2)} 0 0 ${sweep2} ${pbX2.toFixed(2)} ${pbY2.toFixed(2)} L ${end.x.toFixed(2)} ${end.y.toFixed(2)}`;
    }
    const paX = start.x;
  ...
  var _lazyNodaliaPowerFlowCardEditor;
  function loadNodaliaPowerFlowCardEditor() {
    ...
    class NodaliaPowerFlowCardEditor extends HTMLElement {
      ...
      _captureFocusState() {
        ...
        if (!(activeElement instanceof HTMLInputElement || activeElement instanceof HTMLTextAreaElement || activeElement instanceof HTMLSelectElement) || !activeElement.dataset?.field) {
          return null;
        }
        const selector = `[data-field="${CSS.escape(activeElement.dataset.field)}"]`;
        const supportsSelection = (activeElement instanceof HTMLInputElement || activeElement instanceof HTMLTextAreaElement) && activeElement.type !== "checkbox" && typeof activeElement.selectionStart === "number" && typeof activeElement.selectionEnd === "number";
        return {
          selector,
      ...
      _readFieldValue(input) {
        ...
        switch (valueType) {
          ...
          case "rgb-array":
            return String(input.value || "").split(",").map((item) => clamp(Number.parseInt(item.trim(), 10) || 0, 0, 255)).slice(0, 3);
          case "individuals":
    ...
    `;
      ...
      _serializeIndividuals() {
        return resolveIndividualConfigs(this._config).map((entry) => [entry.entity || "", entry.name || "", entry.icon || "", entry.color || ""].join("|")).join("\n");
      }
      _getEntityOptionsMarkup() {
        const sortLoc = window.NodaliaUtils?.editorSortLocale?.(this._hass, this._config?.language ?? "auto") ?? "en";
        const allEntities = Object.keys(this._hass?.states || {}).filter((entityId) => entityId.startsWith("sensor.") || entityId.startsWith("number.") || entityId.startsWith("input_number.")).sort((left, right) => left.localeCompare(right, sortLoc));
        return `
  ...
  var _lazyNodaliaPowerFlowCardVisualEditor;
  function loadNodaliaPowerFlowCardVisualEditor() {
    ...
    `;
      ...
      _renderEntityPickerField(label, field, value, options = {}) {
        ...
        const domains = arrayFromMaybe(options.domains).length ? arrayFromMaybe(options.domains) : POWER_FLOW_ENTITY_DOMAINS;
        return `
      <div class="editor-field ${options.fullWidth ? "editor-field--full" : ""}">
        <span>${escapeHtml(tLabel)}</span>
        <div
    ...
    `;
      ...
      _mountEntityPicker(host) {
        ...
        const domains = String(host.dataset.domains || POWER_FLOW_ENTITY_DOMAINS.join(",")).split(",").map((domain) => domain.trim()).filter(Boolean);
        let control = null;
    ...
    `;
      ...
      <div class="editor">
        ...
        <section class="editor-section">
          ...
          ${this._showTapActionsSection ? `
          <div class="editor-grid editor-grid--stacked">
            ${this._renderSelectField("ed.power_flow.card_tap_action", "tap_action", tapAction2, [
          { value: "none", label: "ed.power_flow.tap_none" },
          { value: "more-info", label: "ed.power_flow.tap_more_info_home" }
        ], { fullWidth: true })}
          </div>
        ...
        <section class="editor-section">
          ...
          <div class="power-flow-individual-list">
            ${individuals.length ? individuals.map((item, index) => this._renderIndividualEditorCard(item, index, individuals.length)).join("") : `<div class="editor-empty-note">${escapeHtml(this._editorLabel("ed.power_flow.individuals_empty"))}</div>`}
          </div>
  ...
  function loadNodaliaPowerFlowCard() {
    ...
    class NodaliaPowerFlowCard extends HTMLElement {
      ...
      _getSecondaryInfoText(nodeConfig, baseState) {
        ...
        if (infoAttribute) {
          ...
          if (rawAttribute === void 0 || rawAttribute === null || rawAttribute === "") {
            return "";
          }
      ...
      _getLayoutConfigStamp() {
        ...
        const individualStamp = resolveIndividualConfigs(this._config).map((entry) => `${entry.entity}|${entry.name}|${entry.icon}|${entry.color}`).join(";");
        return [
          branchStamp,
          individualStamp,
      ...
      _applyHomeDemandDerivedFlows(nodes) {
        ...
        const gridNet = gridImport > 1e-3 ? gridImport : gridExport > 1e-3 ? -gridExport : 0;
      ...
      _getNodes() {
        ...
        const nodes = {
          ...
          individual: showIndividualsOnDiagram ? individualConfigs.map(
      ...
      _buildLines(nodes) {
        ...
        return lineCandidates.map((line) => {
          ...
          const path = line.straight ? buildStraightFlowPath(line.fromNode.position, line.toNode.position, line.fromRadius, line.toRadius) : buildFlowPath(line.fromNode.position, line.toNode.position, line.fromRadius, line.toRadius, pathHints);
          return {
            ...line,
            path,
      ...
      _getDominantColor(lines) {
        const active = [...lines].filter((line) => line.active).sort((left, right) => right.magnitude - left.magnitude);
    ...
    `;
      ...
      _renderNode(node, options = {}) {
        ...
        const unavailableBadge = this._config?.show_unavailable_badge !== false && node.unavailable ? `<span class="power-flow-card__unavailable"><ha-icon icon="mdi:help"></ha-icon></span>` : "";
        ...
        `;
        const labelMarkup = this._config?.show_labels === false ? "" : `<span class="power-flow-card__chip power-flow-card__chip--label">${escapeHtml(node.label)}</span>`;
        const secondaryMarkup = node.secondary ? `<span class="power-flow-card__node-secondary">${escapeHtml(node.secondary)}</span>` : "";
        if (node.kind === "home") {
    ...
    `;
      ...
      _resolveConsumptionChip(period) {
        ...
        const defaultLabel = period === "day" ? this._powerFlowUi("today", "Today") : this._powerFlowUi("month", "Month");
    ...
    `;
      ...
      _renderHomePopupDeviceRow(node, options = {}) {
        ...
        const unavailableBadge = this._config?.show_unavailable_badge !== false && node.unavailable ? `<span class="power-flow-card__unavailable"><ha-icon icon="mdi:help"></ha-icon></span>` : "";
        const labelMarkup = this._config?.show_labels === false ? "" : `<span class="power-flow-card__home-popup-device-name">${escapeHtml(node.label)}</span>`;
        const valueMarkup = this._config?.show_values === false || node.unavailable ? node.unavailable ? `<span class="power-flow-card__chip power-flow-card__chip--value" style="--chip-tint:${escapeHtml(node.color)};"><span>--</span></span>` : "" : `
          <span class="power-flow-card__chip power-flow-card__chip--value" style="--chip-tint:${escapeHtml(node.color)};">
            <span>${escapeHtml(node.valueText)}</span>
            ${node.unitText ? `<span class="power-flow-card__chip-unit">${escapeHtml(node.unitText)}</span>` : ""}
          </span>
    ...
    `;
      ...
      _renderHomeDevicePopup(nodes) {
        ...
        const homeSummaryValue = this._config?.show_values === false ? "" : `${home.valueText}${home.unitText ? ` ${home.unitText}` : ""}`.trim();
        const animations = this._getAnimationSettings();
        const homeSummarySize = Math.round(Math.max(92, parseSizeToPixels(styles.icon?.home_size, 96)));
        const homeSummaryLabel = this._config?.show_labels === false ? "" : `<span class="power-flow-card__chip power-flow-card__chip--label">${escapeHtml(home.label)}</span>`;
        return `
      <div
    ...
    `;
      ...
      _render() {
        ...
        const surfaceAspectCss = upperBandHubOnly ? layoutPreset === "compact" ? "1 / 0.64" : "1 / 0.58" : stripOnlyGridHome ? layoutPreset === "compact" ? "1 / 0.40" : "1 / 0.36" : "1 / 1.04";
      ...
      <ha-card class="power-flow-card power-flow-card--${layoutPreset}">
        ...
        <div class="power-flow-card__content ${shouldAnimateEntrance ? "power-flow-card__content--entering" : ""}" ${this._config?.tap_action === "more-info" ? `data-card-action="primary" role="button" tabindex="0" aria-label="${escapeHtml(titleText || this._powerFlowUi("energy", "Energy"))}"` : ""}>
          ...
          enterDelay: this._getNodeAnimationDelay(nodes.gas)
                  ...
                  ${showIndividualsOnDiagram ? nodes.individual.map((node, index) => this._renderNode(node, {
          layoutPreset,
          animateEntrance: shouldAnimateEntrance,
  ...
  // src/cards/power-flow/standalone.ts
  var _legacyEditorKept = customElements.get(`${EDITOR_TAG}-legacy`) === loadNodaliaPowerFlowCardEditor();
})();
