...
(() => {
  ...
  var SCHEDULE_MIN_BLOCK_MINUTES = 15;
  var SCHEDULE_BLOCK_DRAG_THRESHOLD_PX = 6;
  var HAPTIC_PATTERNS = {
    selection: 8,
    light: 10,
    medium: 16,
  ...
  var RANGE_THUMB_DRAG_THRESHOLD_PX = 8;
  var STEP_BUTTON_COMMIT_DEBOUNCE = 160;
  var DRAFT_CONFIRMATION_TIMEOUT = 4200;
  var DRAFT_CONFIRMATION_RETRY_LIMIT = 1;
  var ENGINE_OVERRIDE_HOLD_HOURS = 2;
  var ENGINE_OVERRIDE_REFRESH_MS = 3e4;
  var LEGACY_CLIMATE_ICON_OFF_COLORS = [
  ...
  var LEGACY_CLIMATE_DIAL_OFF_COLOR = "rgba(255, 255, 255, 0.28)";
  ...
  function normalizeSetpointScheduleConfig(rawSchedule) {
    ...
    const slots = Array.isArray(schedule.slots) ? schedule.slots.map((slot, index) => normalizeSetpointScheduleSlot(slot, index)) : [];
    return {
  ...
  var SETPOINT_SCHEDULE_STORAGE_VERSION = 1;
  var SETPOINT_SCHEDULE_STORAGE_VERSION_PACKED = 2;
  var SETPOINT_SCHEDULE_STORAGE_VERSION_BINARY = 3;
  var SETPOINT_SCHEDULE_INPUT_TEXT_MAX = 255;
  var SETPOINT_SCHEDULE_STORAGE_TIME_QUANTUM = SCHEDULE_TIMELINE_SNAP_MINUTES;
  ...
  function encodeSetpointScheduleBinaryBase64(slots) {
    ...
    slots.forEach((slot, index) => {
      ...
      bytes[offset] = packed >>> 24 & 255;
      bytes[offset + 1] = packed >>> 16 & 255;
      bytes[offset + 2] = packed >>> 8 & 255;
      bytes[offset + 3] = packed & 255;
    });
  ...
  function decodeSetpointScheduleStorageState(rawState) {
    ...
    try {
      ...
      if (Number(parsed.v) === SETPOINT_SCHEDULE_STORAGE_VERSION && Array.isArray(parsed.s)) {
        const enabled = parsed.e !== 0;
        const slots = parsed.s.map((row, index) => {
  ...
  var SETPOINT_SCHEDULE_HA_WEEKDAY = {
    mon: ["mon"],
    tue: ["tue"],
    wed: ["wed"],
    thu: ["thu"],
  ...
  function getClimateScheduleStorageEntityId(entityId, configuredHelper = "") {
    ...
    const slug = String(entityId ?? "").trim().replace(/^climate\./, "").replace(/[^a-z0-9_]+/gi, "_").replace(/^_+|_+$/g, "");
...
${weekdayYaml}
  ...
  function formatTemperature(value, step = 0.5, withUnit = true, hass = null) {
    ...
    if (!Number.isFinite(n)) {
      const u2 = getClimateTemperatureUnit(hass);
      return withUnit ? `-- ${u2}` : "--";
    }
  ...
  function climateDialActionMeta(actionRaw, modeRaw) {
    ...
    if (modeKey === "off") {
      const m2 = getModeMeta("off");
      return { icon: m2.icon, label: m2.label, accent: m2.accent };
    }
    const action = String(actionRaw || "").trim();
    if (action) {
      const m2 = getActionMeta(action);
      const accent = m2.accent != null ? m2.accent : getModeMeta(action).accent;
      return { icon: m2.icon || "mdi:thermostat", label: m2.label, accent };
    }
  ...
  var NodaliaClimateCardEditorLegacy = class extends HTMLElement {
    constructor() {
      super();
    ...
    _getEntityOptionsMarkup() {
      ...
      const climateIds = Object.keys(this._hass?.states || {}).filter((entityId) => entityId.startsWith("climate.")).sort((left, right) => left.localeCompare(right, sortLoc));
      return `
    ...
    _render() {
      ...
      <div class="editor">
        ...
        climateEditorEngineSchedulesActive(this) ? this._editorLabel("ed.engine.climate_managed_hint") : this._editorLabel("ed.climate.schedule_section_hint")
      )}</div>
          </div>
    ...
    `;
      this.shadowRoot.querySelectorAll('input[data-field="entity"]').forEach((input) => input.setAttribute("list", "climate-card-entities"));
    }
  };
  var NodaliaClimateCardEditor = class extends HTMLElement {
    constructor() {
      super();
      this.attachShadow({ mode: "open" });
      this._config = normalizeConfig(STUB_CONFIG);
    ...
    _getClimateEntityOptions() {
      const sortLoc = window.NodaliaUtils?.editorSortLocale?.(this._hass, this._config?.language ?? "auto") ?? "en";
      const options = Object.entries(this._hass?.states || {}).filter(([entityId]) => entityId.startsWith("climate.")).map(([entityId, state]) => {
  ...
  function getClimateDialCenterInsetCss(modeDialButtonCount, tightLayout, compactLayout) {
    const pick = (tightStr, compactStr, regularStr) => tightLayout ? tightStr : compactLayout ? compactStr : regularStr;
  ...
  var NodaliaClimateCard = class extends HTMLElement {
    ...
    constructor() {
      ...
      this._draftTempRange = /* @__PURE__ */ new Map();
    ...
    connectedCallback() {
      ...
      this._detachHostHold = typeof window.NodaliaUtils?.bindHostPointerHoldGesture === "function" ? window.NodaliaUtils.bindHostPointerHoldGesture(this, {
        resolveZone: (event) => {
          ...
          if (path.some(
            (node) => node instanceof HTMLElement && (node.classList?.contains("climate-schedule-expanded") || node.classList?.contains("climate-schedule-agenda__track") || node.dataset?.scheduleBlockId || node.dataset?.scheduleResize || node.dataset?.climateScheduleField || node.dataset?.climateScheduleBackdrop === "true")
          )) {
            return null;
          }
          return path.some((node) => node instanceof HTMLElement && node.dataset?.climateCard === "root") ? "body" : null;
        },
        shouldBeginHold: () => {
    ...
    getCardSize() {
      const showEngineOverride = this._engineOverride?.available === true && this._config?.show_schedule_button !== false;
    ...
    async _setEngineOverrideHold(hours = ENGINE_OVERRIDE_HOLD_HOURS) {
      ...
      const override = { until: new Date(Date.now() + hours * 36e5).toISOString() };
    ...
    _renderEngineOverrideHtml() {
      if (this._engineOverride?.available !== true || this._config?.show_schedule_button === false) {
        return "";
      }
      const applyValues = window.NodaliaUtils?.applyLabelValues || ((text, values) => String(text ?? "").replace(/\{([a-zA-Z0-9_]+)\}/g, (match, token) => Object.prototype.hasOwnProperty.call(values || {}, token) ? String(values[token] ?? "") : match));
      ...
      const statusText = until ? applyValues(this._climateScheduleText("override.activeUntil", "Override until {time}"), {
    ...
    _syncScheduleComposerDraftFromDom() {
      ...
      const domPatchById = /* @__PURE__ */ new Map();
    ...
    _flushScheduleComposerFocusedField() {
      ...
      if (!(active instanceof HTMLInputElement) || !active.dataset?.climateScheduleField || active.dataset.climateScheduleField === "enabled") {
        return;
      }
    ...
    async _submitScheduleComposer() {
      ...
      if (!webhookId) {
        this._scheduleComposerSaving = false;
        this._setScheduleComposerError(
          nativeSaveError ? this._climateScheduleText(
            "errors.nativeFailed",
    ...
    _handleScheduleComposerPointerDown(event, path) {
      ...
      if (path.some(
        (node) => node instanceof HTMLInputElement || node instanceof HTMLTextAreaElement || node instanceof HTMLSelectElement || node instanceof HTMLButtonElement
      )) {
        return false;
      }
    ...
    _getCompactLevel() {
      ...
      if (configuredRows !== null && configuredRows <= 3 || configuredColumns !== null && configuredColumns <= 4) {
        return "tight";
      }
      if (configuredRows !== null && configuredRows <= 4 || configuredColumns !== null && configuredColumns <= 6) {
        return "compact";
      }
      return "default";
    }
    ...
    _getClimateName(state) {
      return this._config?.name || state?.attributes?.friendly_name || this._config?.entity || "Climate";
    }
    _getClimateIcon(state) {
      return this._config?.icon || state?.attributes?.icon || "mdi:thermostat";
    }
    ...
    _getOrderedModeOptions(state) {
      const rawModes = Array.isArray(state?.attributes?.hvac_modes) ? state.attributes.hvac_modes.map((item) => String(item || "").trim()).filter(Boolean) : [];
      const uniqueModes = [...new Set(rawModes)];
      const preferredOrder = ["heat", "cool", "heat_cool", "auto", "dry", "fan_only"];
      const sortLoc = window.NodaliaUtils?.editorSortLocale?.(this._hass, this._config?.language ?? "auto") ?? "en";
      return uniqueModes.filter((mode) => normalizeTextKey(mode) !== "off").sort((left, right) => {
    ...
    _getPreferredOnMode(state) {
      const allModes = Array.isArray(state?.attributes?.hvac_modes) ? state.attributes.hvac_modes.map((item) => String(item || "").trim()).filter(Boolean) : [];
    ...
    _getSetpointWakeHvacMode(state) {
      const modeSet = new Set(
        (Array.isArray(state?.attributes?.hvac_modes) ? state.attributes.hvac_modes : []).map((item) => normalizeTextKey(String(item || "").trim())).filter(Boolean)
      );
      ...
      if (!this._activeDialDrag && stateEarly && this._isDualSetpointRange(stateEarly) && this._selectedRangeThumb) {
        ...
        if (!onRangeThumb) {
          const dialDual = path.find(
            (n) => n instanceof HTMLElement && n.classList?.contains("climate-card__dial") && n.classList?.contains("climate-card__dial--dual-range")
          );
          if (dialDual) {
            this._selectedRangeThumb = null;
            this._render();
    ...
    _onShadowMouseDown(event) {
      ...
      if ((rangeLow || rangeHigh) && this._isDualSetpointRange(this._getState())) {
        ...
        if (!this._activeDialDrag && dial2 && event.button === 0) {
          this._startRangeDialDrag(dial2, rangeLow ? "low" : "high", event.clientX, event.clientY, event, null);
        }
        return;
      }
      const dialHandle = path.find(
    ...
    _onShadowKeyDown(event) {
      ...
      if (target instanceof HTMLInputElement && (target.type === "time" || target.type === "number" || target.type === "text")) {
        event.preventDefault();
        event.stopPropagation();
      }
    }
    _onShadowInput(event) {
      ...
      if (compactInput) {
        ...
        const range = this._getTemperatureRange(state);
        const step = this._getTemperatureStep(state);
        const ratio = clamp((value2 - range.min) / Math.max(range.max - range.min, step), 0, 1);
        ...
        if (valueNode instanceof HTMLElement) {
          valueNode.textContent = formatTemperature(value2, step, true, this._hass);
        }
        if (field2 === "temperature") {
          const targetNode = this.shadowRoot?.querySelector('[data-climate-readout="compact-target"]');
          if (targetNode instanceof HTMLElement) {
            targetNode.textContent = formatTemperature(value2, step, false, this._hass);
          }
          this._queueTemperatureCommit(value2, {
        ...
        if (constrainedValue !== value2) {
          value2 = constrainedValue;
          compactInput.value = String(value2);
          const constrainedRatio = clamp((value2 - range.min) / Math.max(range.max - range.min, step), 0, 1);
          compactInput.closest(".climate-card__compact-slider")?.style.setProperty("--compact-progress", String(constrainedRatio));
          if (valueNode instanceof HTMLElement) {
            valueNode.textContent = formatTemperature(value2, step, true, this._hass);
          }
        }
        const nextPair = field2 === "low" ? { low: value2, high: pair.high } : { low: pair.low, high: value2 };
    ...
    _onShadowBlur(event) {
      ...
      if (!(input instanceof HTMLInputElement) || !input.dataset?.climateScheduleField || input.dataset.climateScheduleField === "enabled") {
        return;
      }
      const slotId = String(input.dataset.scheduleSlotId || "").trim();
      const field = String(input.dataset.climateScheduleField || "").trim();
    ...
    _renderSetpointScheduleDayAgendaRowHtml(day, slots, options = {}) {
      ...
      const daySlots = slots.filter((slot) => slot.day === day && slot.enabled !== false).sort((left, right) => {
        const a = parseScheduleClockMinutes(left.start) ?? 0;
        const b = parseScheduleClockMinutes(right.start) ?? 0;
        return a - b;
      });
      const blocks = daySlots.map((slot) => {
        const layout = getSetpointScheduleBlockLayout(slot);
        const selected = this._scheduleComposerSelectedSlotId === slot.id;
      ...
      const editors = daySlots.map((slot) => this._renderSetpointScheduleSlotEditorHtml(slot, {
        startLabel,
        endLabel,
        tempLabel,
        removeLabel,
    ...
    _renderScheduleComposerHtml(options = {}) {
      ...
      const agendaRows = dayOrder.map(
        (day) => this._renderSetpointScheduleDayAgendaRowHtml(day, schedule.slots, {
    ...
    _render() {
      ...
      const translateClimateMode = (mode) => window.NodaliaI18n?.translateClimateHvacLabel ? window.NodaliaI18n.translateClimateHvacLabel(hass, i18nLang, mode, false) : getModeMeta(mode).label;
      ...
      const showStepControls = config.show_step_controls !== false && (supportsTargetTemperature || supportsNullSetpointCreation);
      const contentColumnGap = showStepControls ? tightLayout ? "8px" : compactLayout ? "9px" : "10px" : effectiveCardGap;
      const effectiveIconSizePx = isCompactCardLayout ? 38 : Math.max(
        48,
      ...
      const modeControlRenderPx = modeDialButtonCount >= 8 ? Math.max(24, Math.round(modeControlSize - 9)) : modeDialButtonCount >= 5 ? Math.max(28, Math.round(modeControlSize - 6)) : modeControlSize;
      const modeDialButtonGap = modeDialButtonCount >= 8 ? "3px" : modeDialButtonCount >= 7 ? "4px" : modeDialButtonCount >= 5 ? tightLayout ? "4px" : "5px" : tightLayout ? "8px" : "10px";
      const interBlockGapPx = showStepControls ? tightLayout ? 16 : compactLayout ? 18 : 20 : tightLayout ? 10 : compactLayout ? 12 : 14;
      ...
      const climateCardMinHeightPx = isCompactCardLayout ? Math.max(
        94,
        Math.round(
          cardPaddingY * 2 + effectiveIconSizePx + (compactControlRowCount ? 16 + compactControlRowCount * 48 + (compactControlRowCount - 1) * 8 : 0) + (modeDialButtonCount || config.show_schedule_button !== false ? 10 + 36 : 0) + overrideBlockPx
        )
      ) : Math.max(
        220,
        Math.round(
          cardPaddingY * 2 + effectiveIconSizePx + dialSizePx + overrideBlockPx + (showStepControls ? stepControlSize : 0) + interBlockGapPx
        )
      );
      const dialCenterGridGap = modeDialButtonCount >= 5 ? tightLayout ? "6px" : compactLayout ? "7px" : "8px" : modeDialButtonCount >= 3 ? tightLayout ? "8px" : compactLayout ? "9px" : "11px" : tightLayout ? "10px" : compactLayout ? "12px" : "15px";
      ...
      const stackedModeControlsGap = modeDialButtonCount >= 5 ? tightLayout ? "6px" : compactLayout ? "6px" : "7px" : tightLayout ? "7px" : "9px";
      ...
      const currentRatio = currentTemperature !== null ? clamp(
        (currentTemperature - temperatureRange.min) / Math.max(temperatureRange.max - temperatureRange.min, temperatureStep),
        0,
        1
      ) : null;
      const currentAngle = currentRatio === null ? null : DIAL_START_ANGLE + currentRatio * DIAL_SWEEP;
      ...
      const dialInlineVars = isRangeMode ? `--climate-thumb-heat-left:${thumbHeatPosition.left}%;--climate-thumb-heat-top:${thumbHeatPosition.top}%;--climate-thumb-cool-left:${thumbCoolPosition.left}%;--climate-thumb-cool-top:${thumbCoolPosition.top}%;--climate-heat-progress:${heatArcLen};--climate-cool-progress:${coolArcLen};--climate-cool-dashoffset:${coolDashOffsetPx}px;` : `--climate-thumb-left:${thumbPosition.left}%;--climate-thumb-top:${thumbPosition.top}%;${showDialCurrentMarker ? `--climate-current-left:${currentMarkerPosition.left}%;--climate-current-top:${currentMarkerPosition.top}%;` : ""}`;
      ...
      const dialPrimaryReadoutHtml = isRangeMode ? escapeHtml(
    ...
    `.trim();
      ...
      const scheduleButtonDialMarkup = showScheduleOnDial ? this._renderSetpointScheduleButtonHtml({ placement: "dial" }) : "";
      const scheduleButtonStepsMarkup = showScheduleInSteps ? this._renderSetpointScheduleButtonHtml({ placement: "steps" }) : "";
      ...
      const dialControlsMarkup = dialModeButtonFragments.length === 0 ? `<div class="climate-card__dial-controls"></div>` : dialControlsStacked ? (() => {
        const rows = buildClimateDialModeButtonRows(dialModeButtonFragments);
        const rowsHtml = rows.map(
          (frags, idx) => `<div class="climate-card__dial-controls-row${idx > 0 ? " climate-card__dial-controls-row--secondary" : ""}">${frags.join("")}</div>`
        ).join("");
        return `
      ...
      const compactScheduleButtonMarkup = showScheduleButton ? this._renderSetpointScheduleButtonHtml({ placement: "steps" }) : "";
    ...
    `;
      ...
      <style>
        ...
        * {
        ...
        .climate-card {
          background: ${isCompactCardLayout ? cardBackground : `radial-gradient(circle at top left, color-mix(in srgb, ${accentColor} 34%, transparent) 0%, transparent 60%),
        ...
        .climate-card::before {
          background: ${isOff ? "linear-gradient(180deg, color-mix(in srgb, var(--primary-text-color) 5%, transparent), rgba(255, 255, 255, 0))" : `linear-gradient(180deg, color-mix(in srgb, ${accentColor} 28%, color-mix(in srgb, var(--primary-text-color) 6%, transparent)), rgba(255, 255, 255, 0))`};
          content: "";
          inset: 0;
          pointer-events: none;
          position: absolute;
        ...
        .climate-card__icon ha-icon {
          ...
          color: ${darkenBubbleIconGlyph ? `color-mix(in srgb, var(--primary-text-color) 56%, ${accentColor})` : isOff ? styles.icon.off_color : styles.icon.on_color};
          display: inline-flex;
        ...
        .climate-card--layout-compact .climate-card__icon {
          background: ${isOff ? "color-mix(in srgb, var(--primary-text-color) 6%, transparent)" : `color-mix(in srgb, ${accentColor} 24%, color-mix(in srgb, var(--primary-text-color) 8%, transparent))`};
          border-radius: 999px;
          height: 38px;
          width: 38px;
        }
      ...
      <ha-card class="climate-card climate-card--${escapeHtml(compactLevel)} ${isCompactCardLayout ? "climate-card--layout-compact" : "climate-card--layout-circular"}" style="--accent-color:${escapeHtml(accentColor)};">
        <div class="climate-card__content" data-climate-card="root">
          <div class="climate-card__hero ${shouldAnimateEntrance ? "climate-card__hero--entering" : ""}">
            ...
            >
              ${entityPicture ? `<img class="climate-card__picture" src="${escapeHtml(entityPicture)}" alt="" loading="lazy" />` : `<ha-icon icon="${escapeHtml(icon)}"></ha-icon>`}
  ...
  var _legacyEditorKept = customElements.get(`${EDITOR_TAG}-legacy`) === NodaliaClimateCardEditorLegacy;
})();
