...
(() => {
  ...
  var model = window.NodaliaRoomSummaryModel;
  var normalizeEntityField = model.normalizeEntityField;
  var collectHubMediaPlayerIds = model.hubMediaPlayerIds;
  var finiteNumber = model.finiteNumber;
  var isUnavailable = model.isUnavailable;
  var stateIsOn = model.stateIsOn;
  var stateIsOpen = model.stateIsOpen;
  var stateIsUnlocked = model.stateIsUnlocked;
  var formatMetric = model.formatMetric;
  ...
  function entityDomain(id) {
    const d = String(id || "").indexOf(".");
    return d > 0 ? String(id).slice(0, d) : "";
  }
  ...
  function normalizeConfig(rawConfig = {}) {
    ...
    if (config.camera) {
      ...
      const cameras = Array.isArray(config.camera_config.cameras) ? config.camera_config.cameras.map((id) => String(id || "").trim()).filter(Boolean) : [];
    ...
    config.media_config.players = Array.isArray(config.media_config.players) ? config.media_config.players.filter(isObject).map((player) => deepClone(player)) : [];
  ...
  function loadNodaliaRoomSummaryCard() {
    ...
    class NodaliaRoomSummaryCard extends HTMLElement {
      ...
      ${panel === "home" ? this._renderHubHome(config, summary, styles, accentColor, collapsed) : this._renderHubPanelContent(panel, config, summary, styles, accentColor)}
    </section>`;
      }
      ...
      _renderHubRail(navItems, activePanel) {
        ...
        navItems.filter((item) => activePanel === "home" || item.id !== activePanel).forEach((item) => {
    ...
    `).join("")}</div>`;
      ...
      _renderHubHeader(config, summary, styles, collapsed = false) {
        ...
        const toggleLabel = collapsed ? this._t("expandDetails", "Expand room details") : this._t("collapseDetails", "Collapse room details");
      ...
      _renderHubStatusChips(config, summary) {
        ...
        if (config.show_camera && config.camera) {
          pushIconChip(
            summary.camera_offline ? "room-hub__metric-bubble--camera room-hub__metric-bubble--camera-offline" : "room-hub__metric-bubble--camera",
            "mdi:cctv",
  ...
  function loadNodaliaRoomSummaryCardEditor() {
    ...
    class NodaliaRoomSummaryCardEditor extends HTMLElement {
      ...
      _onShadowValueChanged(e) {
        const nestedEditor = e.composedPath().find(
        ...
        if (host.dataset.field === "camera") {
          ...
          const cameras = Array.isArray(this._config.camera_config.cameras) ? this._config.camera_config.cameras.map((id) => String(id || "").trim()).filter(Boolean) : [];
