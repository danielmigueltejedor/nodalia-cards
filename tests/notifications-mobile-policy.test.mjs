...
test("transient Engine health failures do not reactivate the legacy package", async () => {
  const { CardClass, sandbox } = loadNotificationsRuntime();
  const serviceCalls = [];
  const postedProfiles = [];
  sandbox.NodaliaBackend = {
...
test("Engine profile write errors do not reactivate the legacy package", async () => {
  const { CardClass, sandbox } = loadNotificationsRuntime();
  const serviceCalls = [];
  sandbox.NodaliaBackend = {
    status: async () => ({ available: true, capabilities: ["notifications_background"] }),
