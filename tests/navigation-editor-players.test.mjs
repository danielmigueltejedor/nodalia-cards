...
test("navigation bar stub and preview render a floating pill like the live card", () => {
  ...
  assert.match(source, /(?:const|let|var) STUB_CONFIG = \{[\s\S]*?border_radius: "32px"/);
  assert.match(source, /(?:const|let|var) STUB_CONFIG = \{[\s\S]*?size: "60px"/);
  ...
  assert.match(source, /playDockEntrance = animations\.enabled\s*&& !inEditMode/);
});
