...
test("Engine inbox dismissals map onto foreground comfort and humidity alert ids", () => {
  const CardClass = loadNotificationsCard();
  const card = new CardClass();
  card.setConfig({});
  ...
  assert.deepEqual(
    asSet(card._nativeDismissalIds("door:binary_sensor.front:on")),
    asSet(["door:binary_sensor.front", "door:binary_sensor.front:on"]),
  );
  assert.deepEqual(
    asSet(card._nativeDismissalIds("hot:sensor.living_room")),
    asSet(["hot:sensor.living_room"]),
  );

  card._mergeNativeDismissed(["hot:sensor.living_room", "humidity_high:sensor.bath", "door:binary_sensor.front"]);
  assert.equal(
    card._isDismissed({ id: "comfort:hot:sensor.living_room:fan.living_room:27" }),
    true,
    "Engine hot:entity dismissals must suppress comfort:hot foreground alerts",
  );
  assert.equal(
    card._isDismissed({ id: "humidity:sensor.bath:high:80" }),
    true,
    "Engine humidity_high:entity dismissals must suppress humidity:entity:high alerts",
  );
  assert.equal(
    card._isDismissed({ id: "door:binary_sensor.front:on" }),
    true,
    "Engine door:entity dismissals must suppress door:entity:state alerts",
  );
