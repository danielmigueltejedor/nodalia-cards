...
function buildBackgroundMobileWebhookPayload(rawConfig) {
  ...
  if (chunks.length > BACKGROUND_MOBILE_MAX_CONFIG_CHUNKS) {
    const error = new Error(`Nodalia Notifications Card: background mobile config needs ${chunks.length} chunks, but the package stores ${BACKGROUND_MOBILE_MAX_CONFIG_CHUNKS}.`);
    error.name = "BackgroundMobileConfigTooLargeError";
    throw error;
  }
  return {
    version: 1,
