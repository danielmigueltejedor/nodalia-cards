...
function buildBackgroundMobileWebhookPayload(rawConfig) {
  ...
  if (chunks.length > BACKGROUND_MOBILE_MAX_CONFIG_CHUNKS) {
    throw new Error(
      `Nodalia Notifications Card: background mobile config uses ${chunks.length} chunks; `
      + `the Home Assistant package stores at most ${BACKGROUND_MOBILE_MAX_CONFIG_CHUNKS}.`,
    );
  }
  return {
    version: 1,
...
class NodaliaNotificationsCard extends HTMLElement {
  ...
  async _syncBackgroundMobileConfig() {
    ...
    try {
      ...
      if (typeof console !== "undefined" && typeof console.warn === "function") {
        console.warn(error?.message || "Nodalia Notifications Card: background mobile config is too large to sync.");
      }
      return false;
    }
  ...
  _smartConfig(kind, entityId = "") {
    ...
    return {
      ...
      mobile: overrideMobile === "inherit" ? (base.mobile || "inherit") : overrideMobile,
    };
  }
...
class NodaliaNotificationsCardEditor extends HTMLElement {
  ...
  async _syncBackgroundMobileConfigFromEditor(config = this._config) {
    ...
    try {
      ...
      if (typeof console !== "undefined" && typeof console.warn === "function") {
        console.warn(error?.message || "Nodalia Notifications Card editor: background mobile config is too large to sync.");
      }
      return false;
    }
    const signature = `${webhookId}:${payload.config_hash}:${payload.chunk_count}`;
