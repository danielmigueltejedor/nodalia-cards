...
const CARD_VERSION = "1.0.0-alpha.80";
...
class NodaliaNotificationsCard extends HTMLElement {
  ...
  _smartAction(kind, fallbackAction = null, fallbackUrlLabel = "", entityId = "") {
    ...
    if (url) {
      return {
        label: config.action_label || fallbackUrlLabel || this._text("actions.open", "Abrir"),
        type: "url",
        url,
      };
    }
  ...
  _render() {
    ...
    const collapsedStackReserve = collapsedStackDepth ? 2 + collapsedStackDepth * 7 : 0;
