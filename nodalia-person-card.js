...
(() => {
  ...
  function normalizeConfig(rawConfig) {
    ...
    const allowedActions = /* @__PURE__ */ new Set(["toggle", "more-info", "service", "navigate", "url", "none"]);
    ...
    `;
      ...
      <style>
        ...
        * {
        ...
        ha-card::before {
          background: ${isUnavailable ? "linear-gradient(180deg, color-mix(in srgb, var(--primary-text-color) 4%, transparent), rgba(255, 255, 255, 0))" : `linear-gradient(180deg, color-mix(in srgb, ${accentColor} 14%, color-mix(in srgb, var(--primary-text-color) 5%, transparent)), rgba(255, 255, 255, 0))`};
          content: "";
          inset: 0;
          pointer-events: none;
          position: absolute;
      ...
      <ha-card class="person-card ${singleRowLayout ? "person-card--single-row" : ""} ${avatarCentered ? "person-card--avatar-centered" : ""}" ${canRunPrimaryAction ? `data-person-action="primary" role="button" tabindex="0" aria-label="${escapeHtml(title)}"` : ""}>
        <div class="person-card__content ${animateWithPicture ? "person-card__content--entering" : ""}">
          <div class="person-card__avatar-track">
            ...
            ${picture ? `<img src="${escapeHtml(picture)}" alt="${escapeHtml(title)}" />` : `<ha-icon icon="${escapeHtml(fallbackIcon)}"></ha-icon>`}
            ${badge ? `<span class="person-card__badge" style="--badge-color:${escapeHtml(badge.color)};"><ha-icon icon="${escapeHtml(badge.icon)}"></ha-icon></span>` : ""}
            </div>
          </div>
          ${showCopyBlock ? `
