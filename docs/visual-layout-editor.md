...
4. Right-click a block for **layout** (only the controls that block uses: e.g. icon → diameter + `styles.icon.*`; presets → accent colors when visible) and **styles** filtered per block. Blocks hidden by config or off-state show layout + a hint only. Right-click empty preview area for **card** styles. Long menus scroll; **Esc** closes the menu. Header **Show/Hide grid** draws column/row lines from the real grid metrics (equal columns + row tracks measured from placed blocks). Removed blocks appear in the palette as a scaled live card thumbnail, not a text label only.
