...
The card ships in the complete Nodalia Cards plugin bundle at `/hacsfiles/nodalia-cards/nodalia-cards.js`. To test a prerelease, select that version through **HACS → Redownload → Need a different version?** instead of pinning a versioned resource filename.
