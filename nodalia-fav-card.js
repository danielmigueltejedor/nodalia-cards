...
class NodaliaFavCard extends HTMLElement {
  ...
  _runAlarmAction(service) {
    ...
    const manualCode = String(this._alarmCodeInput || "").trim();
    if (requiresManualPin && !manualCode) {
    ...
    const code = requiresManualPin ? manualCode : this._getAlarmCodeValue(state);
    if (code) {
      payload.code = code;
    }
  ...
  _performPrimaryAction(state) {
    ...
    switch (tapAction) {
      ...
      case "more-info":
        ...
        break;
      case "service":
        this._callConfiguredService(this._config?.tap_service, this._config?.entity, this._config?.tap_service_data, this._config?.tap_service_target);
        break;
      case "url":
