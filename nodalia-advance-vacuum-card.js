/* Generated from src/cards/advance-vacuum. Do not edit. */
"use strict";
(() => {
  ...
  var CLEANING_SESSION_PENDING_TIMEOUT_MS = 45e3;
  var VACUUM_FEATURE_CLEAN_AREA = 16384;
  var MODE_LABELS = {
    all: "All",
    rooms: "Rooms",
    zone: "Zone",
  ...
  var VACUUM_ONLY_COMBO_PATTERNS = [
    "vacuum_only",
    "only_vacuum",
    "solo_aspirado",
    "solo_aspirar",
  ...
  function getByPath(target, path) {
    return path.split(".").reduce((cursor, key) => cursor === void 0 || cursor === null ? void 0 : cursor[key], target);
  }
  ...
  function sanitizeStyleTree(candidate, fallback) {
    ...
    if (isObject(fallback)) {
      ...
      Object.keys(fallback).forEach((key) => {
        const nextCandidate = isObject(candidate) ? candidate[key] : void 0;
    ...
    return candidate === void 0 ? deepClone(fallback) : candidate;
  }
  ...
  function parseZoneRect(value) {
    if (Array.isArray(value)) {
      if (value.length >= 4 && value.slice(0, 4).every(isFiniteScalar)) {
        const [rawX12, rawY12, rawX22, rawY22] = value.slice(0, 4).map(Number);
        if (rawX12 === rawX22 || rawY12 === rawY22) {
          return null;
        }
        return {
          x1: Math.min(rawX12, rawX22),
          y1: Math.min(rawY12, rawY22),
          x2: Math.max(rawX12, rawX22),
          y2: Math.max(rawY12, rawY22)
        };
      }
    ...
    if ((x2Candidate === void 0 || y2Candidate === void 0) && Number.isFinite(width) && Number.isFinite(height)) {
  ...
  function parsePolygon(value) {
    ...
    return value.map((point) => parsePoint(point)).filter(Boolean);
  }
  function parseOutlines(value) {
    if (!Array.isArray(value) || !value.length) {
      return [];
    }
    const isSinglePolygon = value.every(isPointLike) || isRectangleOutline(value) || value.every(isFiniteScalar) && value.length >= 6 && value.length % 2 === 0;
    if (isSinglePolygon) {
      const polygon2 = parsePolygon(value);
      return polygon2.length >= 3 ? [polygon2] : [];
    }
    const looksNestedCollection = value.every((item) => Array.isArray(item) || isObject(item));
    if (looksNestedCollection) {
      return value.map((polygon2) => parsePolygon(polygon2)).filter((polygon2) => polygon2.length >= 3);
    }
  ...
  function pointInPolygon(point, polygon) {
    ...
    for (let index = 0, previous = polygon.length - 1; index < polygon.length; previous = index, index += 1) {
      ...
      const crossProduct = (point.y - currentPoint.y) * (previousPoint.x - currentPoint.x) - (point.x - currentPoint.x) * (previousPoint.y - currentPoint.y);
      if (Math.abs(crossProduct) < 1e-3 && point.x >= minX && point.x <= maxX && point.y >= minY && point.y <= maxY) {
        return true;
      }
      const intersects = currentPoint.y > point.y !== previousPoint.y > point.y && point.x < (previousPoint.x - currentPoint.x) * (point.y - currentPoint.y) / (previousPoint.y - currentPoint.y || 1e-9) + currentPoint.x;
      if (intersects) {
        inside = !inside;
      }
    }
  ...
  function encodeSharedSessionList(values = []) {
    return arrayFromMaybe(values).map((item) => String(item || "").trim()).filter(Boolean).map((item) => encodeURIComponent(item)).join(",");
  }
  ...
  function encodeSharedSessionZones(zones = []) {
    return arrayFromMaybe(zones).map((zone) => parseZoneRect(zone)).filter(Boolean).map((zone) => `${Math.round(zone.x1)}:${Math.round(zone.y1)}:${Math.round(zone.x2)}:${Math.round(zone.y2)}`).join(";");
  }
  function decodeSharedSessionZones(value = "") {
    return String(value || "").split(";").map((item) => item.trim()).filter(Boolean).map((item) => parseZoneRect(item.split(":").map((part) => Number(part)))).filter(Boolean);
  }
  ...
  function humanizeSelectOptionLabel(value, kind = "generic", hass = null, configLang = null) {
    ...
    const normalized = baseLabel.replaceAll("_", " ").replace(/\s+/g, " ").trim();
    if (!normalized) {
      return "";
    }
  ...
  var CoordinatesConverter = class {
    constructor(calibrationPoints) {
      ...
      const points = arrayFromMaybe(calibrationPoints).map((point) => ({
  ...
  function resolveRoomsFromMapState(hass, entityId) {
    ...
    return Object.entries(rooms).map(([id, room]) => {
      ...
      const outlines = Array.isArray(shapeSource) ? parseOutlines(shapeSource) : (() => {
      ...
      return {
        ...
        outlines,
        outline,
        iconPoint: Number.isFinite(centerX) && Number.isFinite(centerY) ? { x: centerX, y: centerY } : fallbackCenter,
        labelPoint: Number.isFinite(centerX) && Number.isFinite(centerY) ? { x: centerX, y: centerY } : fallbackCenter,
        labelOffsetY: 0
      };
    }).filter((room) => room.id);
  }
  ...
  function loadNodaliaAdvanceVacuumCard() {
    ...
    class NodaliaAdvanceVacuumCard extends HTMLElement {
      ...
      _getAnimationSettings() {
        ...
        return {
          ...
          panelDuration: clamp(Number(configuredAnimations.panel_duration) || DEFAULT_CONFIG.animations.panel_duration, 120, 2e3),
      ...
      _getPressTargetFromEvent(event) {
        return event.composedPath().find((node) => node instanceof HTMLElement && (node.dataset?.zoneHandleIndex || node.dataset?.roomId || node.dataset?.zoneId || node.dataset?.gotoId || node.dataset?.modeId || node.dataset?.headerActionIndex || node.dataset?.manualZoneIndex || node.dataset?.controlAction || node.dataset?.modePresetId || node.dataset?.modeOptionKind || node.dataset?.dockSectionId || node.dataset?.dockActionId || node.dataset?.routineIndex || node.dataset?.customMenuIndex));
      }
      _getVacuumState() {
      ...
      _buildSharedCleaningSessionSnapshot(session, { minimal = false } = {}) {
        ...
        const essentialRoomIds = normalizedSession.activeRoomIds.length ? normalizedSession.activeRoomIds : normalizedSession.selectedRoomIds;
        const essentialZones = normalizedSession.activeZones.length ? normalizedSession.activeZones : normalizedSession.manualZones;
      ...
      _persistSharedCleaningSession(session) {
        ...
        if (!hasEntityTarget && hasWebhookTarget && serializedTrim === this._lastSubmittedSharedCleaningSessionValue) {
          return;
        }
      ...
      _clearSharedCleaningSession() {
        ...
        if ((!currentValue || ["unknown", "unavailable"].includes(normalizeTextKey(currentValue))) && this._lastSubmittedSharedCleaningSessionValue === "") {
          return;
        }
        this._lastSubmittedSharedCleaningSessionValue = "";
      ...
      _restorePersistedCleaningSessionState() {
        ...
        this._selectedManualZoneIndex = this._manualZones.length > 0 ? clamp(this._selectedManualZoneIndex, 0, this._manualZones.length - 1) : -1;
      ...
      _persistCurrentCleaningSessionState(activeMode = this._activeMode, { markSelectionChange = false, selectionUpdatedAt = null } = {}) {
        const nextSelectionUpdatedAt = markSelectionChange ? this._markSelectionInteraction(selectionUpdatedAt || Date.now()) : Number(this._selectionUpdatedAt || 0);
      ...
      _setPendingRoomCleaningResume(roomIds = [], repeats = this._repeats) {
        this._pendingRoomCleaningResumeRoomIds = [...new Set(arrayFromMaybe(roomIds).map((item) => String(item || "").trim()).filter(Boolean))];
      ...
      _getPendingRoomCleaningResumeState(persistedSession = this._readStoredCleaningSession()) {
        ...
        return {
          ...
          repeats: clamp(
            Number(
              (hasInMemoryPendingResume ? this._pendingRoomCleaningResumeRepeats : 0) || persistedSession?.resumeRepeatsAfterZone || this._repeats || 1
            ),
            1,
            9
          )
      ...
      _isCleaningSessionPendingStart(session = this._readStoredCleaningSession()) {
        ...
        const timeoutMs = typeof CLEANING_SESSION_PENDING_TIMEOUT_MS === "number" ? CLEANING_SESSION_PENDING_TIMEOUT_MS : 45e3;
      ...
      _extractRoomIdsFromValue(value, depth = 0) {
        if (value === null || value === void 0 || depth > 5) {
          return [];
        }
        if (Array.isArray(value)) {
        ...
        if (isObject(value)) {
          const directCandidates = [
            ...
            value.areaId,
            value.number
          ].filter((candidate) => candidate !== null && candidate !== void 0 && candidate !== "");
      ...
      _normalizeReportedRoomIds(values = []) {
        ...
        const resolved = this._extractRoomIdsFromValue(values).map((value) => {
          ...
          const suffixMatches = rooms.map((room) => String(room.id)).filter((roomId) => roomId.endsWith(`_${value}`));
      ...
      _getExternalRoomTrackingSnapshot(hass = this._hass) {
        ...
        roomIds.forEach((entityId) => {
          ...
          const configuredValue = entityId === explicitRoomEntityId && explicitAttribute ? attributes[explicitAttribute] : void 0;
          const values = configuredValue !== void 0 ? [configuredValue] : [
          ...
          if (configuredValue === void 0 && (isCurrentSource || entityId === explicitRoomEntityId)) {
      ...
      _getCurrentVacuumRoomId(state = this._getVacuumState()) {
        const mapState = this._getMapState();
        const external = this._getExternalRoomTrackingSnapshot();
        const candidate = mapState?.attributes?.vacuum_room ?? mapState?.attributes?.current_room ?? mapState?.attributes?.current_room_id ?? mapState?.attributes?.current_area ?? state?.attributes?.vacuum_room ?? state?.attributes?.room_id ?? state?.attributes?.current_room_id ?? state?.attributes?.current_area ?? external.currentId;
      ...
      _extractZoneRectsFromValue(value) {
        ...
        if (Array.isArray(value)) {
          const directZone2 = parseZoneRect(value);
          if (directZone2) {
            return [directZone2];
          }
      ...
      _isCleaningSessionActive(state = this._getVacuumState(), persistedSession = this._readStoredCleaningSession()) {
        const hasTrackedWork = Boolean(
          this._activeCleaningRoomIds.length || this._activeCleaningZones.length || persistedSession?.activeRoomIds?.length || persistedSession?.activeZones?.length || this._getReportedCleaningRoomIds(state).length
        );
        const isDockMaintenanceInterlude = hasTrackedWork && (this._isWashingMops(state) || this._isDryingMops(state) || this._isAutoEmptying(state));
        return this._isCleaning(state) || this._isPaused(state) || this._isReturning(state) || isDockMaintenanceInterlude || this._isCleaningSessionPendingStart(persistedSession);
      }
      _isRoomCleaningSessionActive(state = this._getVacuumState()) {
        const persistedSession = this._readStoredCleaningSession();
        const reportedRoomIds = this._getReportedCleaningRoomIds(state);
        return this._matchesActivity(state, ["segment_cleaning", "room_cleaning"]) || this._isCleaningSessionActive(state, persistedSession) && (reportedRoomIds.length > 0 || this._activeCleaningSessionMode === "rooms" || persistedSession?.mode === "rooms" || this._activeCleaningRoomIds.length > 0);
      }
      ...
      _hasMixedRoomZoneCleaningSession(persistedSession = this._readStoredCleaningSession(), reportedZones = []) {
        ...
        return Boolean(
          (Array.isArray(reportedZones) ? reportedZones.length : 0) || this._activeCleaningZones.length || arrayFromMaybe(persistedSession?.activeZones).length
        );
      }
      _attemptPendingRoomCleaningResume(state = this._getVacuumState(), persistedSession = this._readStoredCleaningSession()) {
        ...
        if (!this._isReturning(state) && !this._isDocked(state) && !this._matchesActivity(state, ["idle", "stopped", "ready"])) {
          return false;
        }
        const roomIds = pendingResume.roomIds.map((id) => String(id || "").trim()).filter(Boolean);
        if (!roomIds.length) {
      ...
      _syncActiveCleaningSession(state = this._getVacuumState()) {
        ...
        const isReportedCleaningSessionActive = this._isCleaning(state) || this._isPaused(state) || this._isReturning(state) || reportedRoomIds.length > 0 && (this._isWashingMops(state) || this._isDryingMops(state) || this._isAutoEmptying(state));
        ...
        const hadTrackedCleaningSession = Boolean(
          this._activeCleaningSessionMode || persistedSession?.mode || this._activeCleaningRoomIds.length || this._activeCleaningZones.length || persistedSession?.activeRoomIds?.length || persistedSession?.activeZones?.length || this._pendingCleaningSessionStartAt || persistedSession?.pendingStartAt || this._getPendingRoomCleaningResumeState(persistedSession).roomIds.length
        );
        ...
        if (reportedRoomIds.length && isCleaningSessionActive) {
          this._activeCleaningRoomIds = hasMixedRoomZoneCleaningSession && trackedRoomIds.length ? trackedRoomIds : reportedRoomIds;
        } else if (isCleaningSessionActive && isRoomCleaning && currentRoomId && !hasMixedRoomZoneCleaningSession) {
          this._activeCleaningRoomIds = [.../* @__PURE__ */ new Set([currentRoomId, ...trackedRoomIds])];
        ...
        if (!isCleaningSessionActive) {
          ...
          if (!this._selectedRoomIds.length && !this._selectedPredefinedZoneIds.length && !this._manualZones.length) {
      ...
      _getRenderSignature(hass = this._hass) {
        ...
        const roomTrackingSignature = [.../* @__PURE__ */ new Set([
        ...
        const needsModeDescriptorSignature = this._activeUtilityPanel === "modes" || this._activeUtilityPanel === "dock" || ["rooms", "zone", "goto"].includes(this._activeMode) || ["rooms", "zone", "goto"].includes(displayModeId);
        ...
        const routineSignature = this._activeMode === "routines" || this._activeUtilityPanel === "modes" ? this._getRoutineItems(state).map((item) => {
        ...
        const sections = [
          ...
          },
          {
            ...
            values: [
              ...
              suctionDescriptor ? `${suctionDescriptor.service}:${suctionDescriptor.target}:${suctionDescriptor.current}:${suctionDescriptor.options.join("|")}` : "",
              mopDescriptor ? `${mopDescriptor.service}:${mopDescriptor.target}:${mopDescriptor.current}:${mopDescriptor.options.join("|")}` : "",
              mopModeDescriptor ? `${mopModeDescriptor.service}:${mopModeDescriptor.target}:${mopModeDescriptor.current}:${mopModeDescriptor.options.join("|")}` : "",
              ...
              typeof window !== "undefined" ? window.NodaliaI18n?.resolveLanguage?.(
      ...
      _getRoutineItems(state = this._getVacuumState()) {
        return normalizeRoutineItems(this._config?.routines).filter((item) => this._isMenuItemVisible(item, state));
      }
      _getDisplayCleaningModeId() {
        return ["rooms", "zone", "goto"].includes(this._activeMode) ? this._activeMode : this._activeCleaningSessionMode || this._activeMode || "all";
      }
      ...
      _getAvailableModes() {
        ...
        const hasZones = this._supportsMapActionKind("zone") && (this._config?.allow_zone_mode !== false || this._getPredefinedZones().length > 0 || Boolean(resolveLegacyMode(this._config, "vacuum_clean_zone")));
      ...
      _getDockControlDescriptor(definition, state = this._getVacuumState()) {
        ...
        if (toggleEntity) {
          const isActive2 = this._isBooleanEntityOn(toggleEntity);
          return {
            id: definition.id,
            label: isActive2 ? activeLabel : inactiveLabel,
            icon: isActive2 ? definition.active_icon || definition.icon : definition.icon,
            target: toggleEntity,
            active: isActive2,
            type: "toggle"
          };
      ...
      _setModeOption(kind, value, state = this._getVacuumState(), options = {}) {
        ...
        if (kind === "mop_mode") {
          const descriptor2 = this._getMopModeDescriptor(state);
        ...
        switch (presetId) {
          ...
          case "vacuum_mop":
            return {
              ...
              mopMode: this._getCleaningComboOptionForPreset(mopModeDescriptor, "vacuum_mop") || this._getPresetDefaultOption(mopModeDescriptor, ["standard", "estandar", "normal", "default"])
            };
          case "vacuum":
            return {
              suction: this._getPresetDefaultOption(suctionDescriptor, ["balanced", "equilibrado", "standard", "normal"], {
              ...
              mopMode: this._getCleaningComboOptionForPreset(mopModeDescriptor, "vacuum") || this._getPresetDefaultOption(mopModeDescriptor, ["standard", "estandar", "normal", "default"])
            };
          case "mop":
            return {
              ...
              mopMode: this._getCleaningComboOptionForPreset(mopModeDescriptor, "mop") || this._getPresetDefaultOption(mopModeDescriptor, ["standard", "estandar", "normal", "default"])
            };
          case "custom":
            return {
      ...
      _detectModePanelPreset(state = this._getVacuumState()) {
        ...
        const cleaningComboMode = this._getCleaningComboModeFromValue(
          this._descriptorSupportsCleaningCombo(mopModeDescriptor) ? mopModeCurrent : this._descriptorSupportsCleaningCombo(mopDescriptor) ? mopCurrent : ""
        );
        if (this._isSharedSmartMode(suctionCurrent) && this._isSharedSmartMode(mopCurrent) && (!mopModeCurrent || this._isSharedSmartMode(mopModeCurrent))) {
          return "smart";
        }
        if (this._isCustomModeValue(suctionCurrent) || this._isCustomModeValue(mopCurrent) || this._isCustomModeValue(mopModeCurrent)) {
          return "custom";
        }
      ...
      _isAmbiguousVacuumMopPreset(detectedPreset, state = this._getVacuumState()) {
        ...
        return !mopDescriptor?.current || this._isSharedSmartMode(mopDescriptor.current) || this._isCustomModeValue(mopDescriptor.current) || this._isMopIntensityDescriptor(mopDescriptor);
      }
      _getActiveModePanelPreset(state = this._getVacuumState()) {
        ...
        const stableDetectedPreset = isAmbiguousVacuumMopPreset && fallbackPreset ? fallbackPreset : detectedPreset;
        if (stableDetectedPreset && stableDetectedPreset !== "custom" && !isFrozenPresetContext) {
          this._lastResolvedModePanelPreset = stableDetectedPreset;
        }
        const shouldKeepFrozenPreset = Boolean(this._dockedModePanelPreset) && (isFrozenPresetContext || isAmbiguousVacuumMopPreset || !stableDetectedPreset || stableDetectedPreset === "custom" || stableDetectedPreset === this._dockedModePanelPreset);
      ...
      _vacuumOutlineToSvgPoints(points) {
        return points.map((point) => this._converter.vacuumToMap(point.x, point.y)).map((point) => `${point.x.toFixed(1)},${point.y.toFixed(1)}`).join(" ");
      }
      _vacuumOutlineToCssPolygon(points) {
        return points.map((point) => this._converter.vacuumToMap(point.x, point.y)).map((point) => {
      ...
      _getConfiguredMapMode(actionKind) {
        const templates = actionKind === "rooms" ? ["vacuum_clean_segment"] : actionKind === "zone" ? ["vacuum_clean_zone", "vacuum_clean_zone_predefined"] : ["vacuum_goto", "vacuum_goto_predefined"];
        return templates.map((template) => resolveLegacyMode(this._config, template)).find((mode) => isObject(mode?.service_call_schema)) || null;
      }
      ...
      _isBuiltInMapService(service, serviceData = {}, target = null, profile = this._getVacuumPlatformProfile()) {
        ...
        const requestedEntityIds = [serviceData?.entity_id, target?.entity_id].flatMap((value) => Array.isArray(value) ? value : [value]).map((value) => String(value || "").trim()).filter(Boolean);
        return Boolean(configuredEntityId) && requestedEntityIds.length > 0 && requestedEntityIds.every((entityId) => entityId === configuredEntityId);
      }
      _callConfiguredMapModeService(actionKind, selection, repeats = 1, point = null) {
        ...
        const normalizeSelectionItem = (item) => Array.isArray(item) ? item.map(normalizeSelectionItem) : typeof item === "string" && /^-?\d+(?:\.\d+)?$/.test(item.trim()) ? Number(item) : item;
        const normalizedSelection = normalizeSelectionItem(selection);
        const unwrapped = Array.isArray(normalizedSelection) ? normalizedSelection.flat(Infinity).map((item) => String(item)).join(",") : String(normalizedSelection ?? "");
        const context = {
          ...variables,
          entity_id: this._config.entity,
      ...
      _callRoomCleaningService(roomIds, repeats = 1) {
        ...
        const canUseNativeCleanArea = profile === "home_assistant" || profile === "roborock" && repeats === 1 && supportsNativeCleanArea && Boolean(this._hass?.services?.vacuum?.clean_area) && cleaningAreaIds.length === roomIds.length;
        ...
        const params = profile === "send_command" ? Array.from({ length: repeats }, () => numericOrStringIds).flat() : [{ segments: numericOrStringIds, repeat: repeats }];
        return this._callInternalService("vacuum.send_command", {
          entity_id: this._config.entity,
      ...
      _callZoneCleaningService(zones, repeats = 1) {
        ...
        if (["dreame", "xiaomi_miio"].includes(profile)) {
          const service = profile === "dreame" ? "dreame_vacuum.vacuum_clean_zone" : "xiaomi_miio.vacuum_clean_zone";
      ...
      _toggleRoomSelection(roomId) {
        ...
        this._selectedRoomIds = this._selectedRoomIds.includes(roomId) ? this._selectedRoomIds.filter((id) => id !== roomId) : [...this._selectedRoomIds, roomId];
      ...
      async _runMapAction() {
        ...
        try {
          ...
          if (this._activeMode === "rooms") {
            const roomIds = this._selectedRoomIds.map((id) => String(id || "").trim()).filter(Boolean);
            if (!roomIds.length) {
          ...
          if (this._activeMode === "zone" && selectedZones.length) {
            const isTransientZoneAddition = Boolean(this._transientZoneReturnMode) && (this._isCleaning(state) || this._isPaused(state) || this._isReturning(state) || this._isRoomCleaningSessionActive(state));
            this._freezeCurrentModePanelPreset(state);
    ...
    `;
      ...
      _onShadowPointerMove(event) {
        ...
        if (this._pendingTouchZoneStart && event.pointerType === "touch" && event.pointerId === this._pendingTouchZoneStart.pointerId) {
      ...
      _onShadowPointerUp(event) {
        ...
        if (surface && event.pointerId !== void 0) {
          try {
      ...
      _getRoomMarkerPlacements(rooms, markerSize, labelSize, iconSize) {
        ...
        const placedRects = [];
        const orderedRooms = rooms.filter((room) => this._getPrimaryRoomOutline(room).length >= 3).map((room) => {
          const primaryOutline = this._getPrimaryRoomOutline(room);
          return {
            room,
        ...
        }).filter((item) => item.area > 0).sort((left, right) => left.area - right.area);
        orderedRooms.forEach(({ room, preferredAnchor }) => {
          ...
          const fallbackAnchor = preferredAnchor && Number.isFinite(preferredAnchor.x) && Number.isFinite(preferredAnchor.y) ? preferredAnchor : this._getPrimaryRoomOutline(room).length >= 3 ? centroid(this._getPrimaryRoomOutline(room)) : null;
          ...
          candidates.forEach((candidate, index) => {
            ...
            const overflow = Math.max(0, 8 - rect.left) + Math.max(0, 8 - rect.top) + Math.max(0, rect.right - (viewportWidth - 8)) + Math.max(0, rect.bottom - (viewportHeight - 8));
            const overlap = placedRects.reduce((acc, placedRect) => acc + rectIntersectionArea(rect, placedRect), 0);
            const distancePenalty = preferredAnchor ? Math.hypot(candidate.x - preferredAnchor.x, candidate.y - preferredAnchor.y) * 0.01 : 0;
            const score = overlap * 1e3 + overflow * 100 + index * 4 + distancePenalty;
            if (score < bestScore) {
              bestScore = score;
              bestPlacement = {
      ...
      _renderRoomMarkers(rooms) {
        ...
        >
          ...
          ${this._config?.show_room_labels === false ? "" : `<span>${escapeHtml(room.label || room.id)}</span>`}
        </button>
      `;
        }).join("");
      }
      _renderRoomSelectionHighlights(rooms, highlightedRoomIds, mapImageUrl, modeId = this._activeMode) {
        if (modeId !== "rooms" || !mapImageUrl) {
          return "";
        }
        const highlights = rooms.filter((room) => highlightedRoomIds.has(String(room.id))).flatMap((room) => room.outlines.map((outline, index) => ({
    ...
    `;
      ...
      _renderZoneSelectionHighlights(zones, mapImageUrl, modeId = this._activeMode) {
        if (modeId !== "rooms" || !Array.isArray(zones) || !zones.length || !mapImageUrl) {
          return "";
        }
        const highlights = zones.map((zone, index) => ({
    ...
    `;
      ...
      _renderManualZoneEditors() {
        ...
        return this._manualZones.map((zone, index) => {
          ...
          const handleMarkup = selected ? handles.map((handle) => {
    ...
    `;
      ...
      _renderModePanel(state) {
        ...
        `,
          this._activeMode === "goto" ? `<div class="advance-vacuum-card__selection-chip"><strong>${this._gotoPoint ? "1" : "0"}</strong><span>${escapeHtml(u?.pointWord ?? "punto")}</span></div>` : ""
        ].filter(Boolean).join("");
    ...
    `).join("");
      ...
      _renderDockPanel(state) {
        ...
        const availableSections = DOCK_PANEL_SECTIONS.filter((section) => section.id === "control" ? controlDescriptors.length > 0 : settingDescriptors.length > 0);
    ...
    `;
      ...
      _render() {
        ...
        try {
          ...
          const hasPendingZoneSelection = currentMode.id === "zone" && (this._selectedPredefinedZoneIds.length > 0 || this._manualZones.length > 0);
          const primaryButtonIcon = hasPendingZoneSelection ? "mdi:check" : this._isCleaning(state) ? "mdi:pause" : "mdi:play";
          const primaryButtonTitle = hasPendingZoneSelection ? isCleaningSessionActive ? advanceVacuumStrings?.actions?.addZoneToClean || "Add zone to cleaning" : advanceVacuumStrings?.actions?.cleanZone || "Clean zone" : advanceVacuumStrings?.actions?.run || "Run";
          ...
          const utilityPanelMarkup = isRoutinesMode ? "" : this._activeUtilityPanel === "modes" ? this._renderModePanel(state) : this._activeUtilityPanel === "dock" ? this._renderDockPanel(state) : "";
      ...
      <ha-card class="advance-vacuum-card ${shouldAnimateEntrance ? "advance-vacuum-card--entering" : ""}">
        ...
        ${utilityPanelMarkup ? `<div class="advance-vacuum-card__utility-panel-slot">${utilityPanelMarkup}</div>` : ""}
        </div>
      </ha-card>
    `;
