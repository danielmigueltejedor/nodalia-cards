...
(() => {
  ...
  var NodaliaGraphCardEditorLegacy = class extends HTMLElement {
    constructor() {
      super();
    ...
    _serializeEntities() {
      return this._config.entities.map((entry) => [entry.entity || "", entry.name || "", entry.color || ""].join("|")).join("\n");
    }
    _render() {
      if (!this.shadowRoot) {
        return;
  ...
  var NodaliaGraphCardEditor = class extends HTMLElement {
    constructor() {
      super();
      this.attachShadow({ mode: "open" });
  ...
  var NodaliaGraphCard = class extends HTMLElement {
  ...
  // src/cards/graph/standalone.ts
  var _legacyEditorKept = customElements.get(`${EDITOR_TAG}-legacy`) === NodaliaGraphCardEditorLegacy;
})();
