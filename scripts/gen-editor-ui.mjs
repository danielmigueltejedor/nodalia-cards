...
(() => {
  const ROW_LANGS = ${JSON.stringify(EDITOR_ROW_LANGS)};
  const ROWS_JSON = ${JSON.stringify(JSON.stringify(compactRows))};
  const EDITOR_CATALOG_JSON = ${JSON.stringify(JSON.stringify(compactEditorCatalog))};
  let ROWS_CACHE = null;
  let MAP_CACHE = null;
  ...
  function getRows() {
    if (!ROWS_CACHE) {
      ROWS_CACHE = JSON.parse(ROWS_JSON).map(values => Object.fromEntries(
        ROW_LANGS.map((lang, index) => [lang, values[index] ?? ""]),
      ));
    }
    return ROWS_CACHE;
  }
  ...
  function getEditorCatalog() {
    if (!EDITOR_CATALOG_CACHE) {
      const compact = JSON.parse(EDITOR_CATALOG_JSON);
      EDITOR_CATALOG_CACHE = {};
      compact.langs.forEach((lang, langIndex) => {
        const values = compact.values[langIndex] || [];
        EDITOR_CATALOG_CACHE[lang] = Object.fromEntries(
          compact.keys.map((key, keyIndex) => [key, values[keyIndex] ?? ""]),
        );
      });
